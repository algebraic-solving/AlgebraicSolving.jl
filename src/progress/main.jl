...
export ProgressBar, update!, next!, finish!
...
@kwdef mutable struct ProgressBar
    ...
    enabled::Bool = true
    finished::Bool = false
    terminal::Terminals.TTYTerminal = Terminals.TTYTerminal(string(), stdin, stdout, stderr)
    ...
    lock(progress_bar_lock) do
        ...
        for _ in 1:prog.offset
            if prog.i == 0
                ...
                Terminals.cmove_line_down(prog.terminal)
            end
        end
        Terminals.clear_line(prog.terminal)
        print(buffer)
        Terminals.cmove_left(prog.terminal, length(buffer))
        if prog.finished
            if prog.offset == 0
                println()
            end
        ...
        for _ in 1:prog.offset
            Terminals.cmove_line_up(prog.terminal)
        end
    end
end
