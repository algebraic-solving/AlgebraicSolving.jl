@testset "Interfaces -> AbstractAlgebra" begin
    ...
    R, (x,y,z) = PolynomialRing(GF(2147483659),["x","y","z"], ordering=:degrevlex)
    F = [x^2+1-3, x*y-z, x*z^2-3*y^2]
    # prime is bigger than 2^31, should throw an error
    @test_throws ErrorException AlgebraicSolving.convert_to_msolve(F)
    R, (x,y,z) = PolynomialRing(GF(101),["x","y","z"], ordering=:degrevlex)
    F = [x^2+1-3, x*y-z, x*z^2-3*y^2]
    res = AlgebraicSolving.convert_to_msolve(F)
    @test AlgebraicSolving.convert_finite_field_gb_to_abstract_algebra(Int32(3), res..., R) == F
end
